...
export function App() {
  ...
  const getTopListInfo = async (onSuccess?: (receipt: any) => void) => {
    const data = await readContract({
      ...
      method:
        "function getTopListInfo() view returns (uint256[10], uint256[10], address[10], uint256)",
      params: [],
    });
    onSuccess?.(data);
  };
  ...
  const getPlayerAllAssets = async (onSuccess?: (receipt: any) => void) => {
    ...
    if (account) {
      const data = await readContract({
        contract: getGameContract(currentChain),
        method:
          "function getPlayerAllAssets(address player) view returns (uint256 gold, uint256 diamond)",
        params: [account],
      });
      onSuccess?.(data);
    }
  ...
  const getPlayerLastLotteryResult = async (
    ...
    if (account) {
      const data = await readContract({
        contract: getGameContract(currentChain),
        method:
          "function getPlayerLastLotteryResult(address player) view returns (uint256 itemType, uint256 num)",
        params: [account],
      });
      onSuccess?.(data);
    }
  ...
  const getPlayerAllWeaponInfo = async (onSuccess?: (receipt: any) => void) => {
    ...
    if (account) {
      const data = await readContract({
        contract: getGameContract(currentChain),
        method:
          "function getPlayerAllWeaponInfo(address player) view returns (uint256[] weaponIdList, uint256[] weaponLevelList)",
        params: [account],
      });
      onSuccess?.(data);
    }
  ...
  const getPlayerAllSkinInfo = async (onSuccess?: (receipt: any) => void) => {
    ...
    if (account) {
      const data = await readContract({
        contract: getGameContract(currentChain),
        method:
          "function getPlayerAllSkinInfo(address player) view returns (uint256[] skinIdList, uint256[] skinLevelList)",
        params: [account],
      });
      onSuccess?.(data);
    }
